...
function buildMaps(month_value) {
    ...
    function successHandle(response){
        ...
        for (var resp in response){ 
            ...
            cancelled_flights = response[resp].canc_pct
            airport_name = resp
            ...
            canc_flight_markers.push(
                L.circle(lat_lng, {
            stroke: true,
            fillOpacity: 0.75,
            color: "red",
            fillColor: "red",
            radius: ((cancelled_flights*total_flights)/100)*1.5,
            }).bindTooltip("Name:" + name + "<hr> Location: " + place +" <hr> cancelled %: " + cancelled_flights.toFixed(2))
            )                  
        }
        ...
        console.log(`canc_mark`,canc_flight_markers)
        ...
        var cancf = L.layerGroup(canc_flight_markers)

        //create an overlay object
        var overlayMaps = {
            "Total Arrivals": flights,
            "Total Delays":delay,
            "Total Cancelled":cancf
        }
        ...
        // var myMap = L.map("map").setView([37.0902, -95.7129], 18,{layers:[darkmap,flights,delay]})

        var myMap = L.map("map",{
            center: [37.0902, -95.7129],
            zoom:5,
            layers:[darkmap,flights,delay,cancf]
        })
